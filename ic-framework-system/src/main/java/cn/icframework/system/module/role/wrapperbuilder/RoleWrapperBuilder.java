package cn.icframework.system.module.role.wrapperbuilder;
...
public class RoleWrapperBuilder extends BasicWrapperBuilder<RoleDef> {
     ...
     */
    @Override
    protected SqlWrapper list(QueryParams params, RoleDef def) {
        SqlWrapper sqlWrapper = SELECT().FROM(def).WHERE(def.su.eq(false));
        params.forEach((key, rv) -> {
            ...
            switch (key) {
                ...
                case "status" -> sqlWrapper.WHERE(def.status.in(List.of(values)));
     ...
     */
    @Override
    protected QueryField<?> doSort(OrderItem orderItem, RoleDef def) {
     ...
     */
    @Override
    protected List<DefaultOrderBy> defaultSort(RoleDef def) {
        return new OrderBuilder().orderDesc(def.userType, def.name).build();
    }
