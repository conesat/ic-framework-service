package cn.icframework.system.module.noticereceiver.wrapperbuilder;
...
public class NoticeReceiverWrapperBuilder extends BasicWrapperBuilder<NoticeReceiverDef> {
     ...
     * 返回一个数据库定义对象
     */
    public NoticeReceiverWrapperBuilder() {
        super(NoticeReceiverDef.table());
    }
     ...
     */
    @Override
    protected SqlWrapper list(QueryParams params, NoticeReceiverDef def) {
        UserDef userDef = UserDef.table();
        SqlWrapper sqlWrapper = SELECT(def._all,
                userDef.name.as(NoticeReceiverVO::getUserName),
                userDef.avatarFileUrl.as(NoticeReceiverVO::getAvatarFileUrl))
                .FROM(def)
        ...
        params.forEach((key, rv) -> {
            ...
            switch (key) {
                ...
                case "noticeId" -> sqlWrapper.WHERE(def.noticeId.eq(value));
            }
        });
        return sqlWrapper;
    }
     ...
     */
    @Override
    protected QueryField<?> doSort(OrderItem orderItem, NoticeReceiverDef def) {
        return switch (orderItem.getSortBy()) {
            case "readTime" -> def.readTime;
            default -> null;
        };
    }
     ...
     */
    @Override
    protected List<DefaultOrderBy> defaultSort(NoticeReceiverDef def) {
        // 使用以下方法构建默认排序
         return new OrderBuilder().orderAsc(def.readTime).build();
    }
}
