package cn.icframework.system.module.dic.wrapperbuilder;
...
import cn.icframework.system.module.dic.def.DicDef;
...
public class DicWrapperBuilder extends BasicWrapperBuilder<DicDef> {
     ...
     */
    @Override
    protected SqlWrapper list(QueryParams params, DicDef def) {
        SqlWrapper sqlWrapper = SELECT_FROM(def);
        params.forEach((key, rv) -> {
            Object value = rv.getValue(); // 单个参数
            Object[] values = rv.getValues(); // 数组参数
            switch (key) {
                case ParamsConst.SEARCH_KEY -> sqlWrapper.WHERE(def.dicKey.like(value).or().dicVal.like(value));
            }
        });
        return sqlWrapper;
    }
     ...
     */
    @Override
    protected QueryField<?> doSort(OrderItem orderItem, DicDef def) {
        return switch (orderItem.getSortBy()) {
            case "dicKey" -> def.dicKey;
            case "dicVal" -> def.dicVal;
     ...
     */
    @Override
    protected List<DefaultOrderBy> defaultSort(DicDef def) {
