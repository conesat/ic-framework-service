package cn.icframework.system.module.setting.wrapperbuilder;
...
import cn.icframework.system.module.setting.def.SettingDef;
...
public class SettingWrapperBuilder extends BasicWrapperBuilder<SettingDef> {
    public SettingWrapperBuilder() {
        super(SettingDef.table());
    }
     ...
     */
    @Override
    protected SqlWrapper list(QueryParams params, SettingDef def) {
        SqlWrapper sqlWrapper = SELECT_FROM(def);
        params.forEach((key, rv) -> {
            Object value = rv.getValue(); // 单个参数
            Object[] values = rv.getValues(); // 数组参数
            switch (key) {
                // case ParamsConst.SEARCH_KEY -> sqlWrapper.WHERE(TSetting.name.like(value));
            }
        });
        return sqlWrapper;
    }
     ...
     */
    @Override
    protected QueryField<?> doSort(OrderItem orderItem, SettingDef def) {
     ...
     */
    @Override
    protected List<DefaultOrderBy> defaultSort(SettingDef def) {
