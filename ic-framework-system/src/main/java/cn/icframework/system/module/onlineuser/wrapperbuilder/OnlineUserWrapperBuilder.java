package cn.icframework.system.module.onlineuser.wrapperbuilder;
...
public class OnlineUserWrapperBuilder extends BasicWrapperBuilder<OnlineUserDef> {
     ...
     * 返回一个数据库定义对象
     */
    public OnlineUserWrapperBuilder() {
        super(OnlineUserDef.table());
    }
     ...
     */
    @Override
    protected SqlWrapper list(QueryParams params, OnlineUserDef def) {
        UserDef userDef = UserDef.table();
        SqlWrapper sqlWrapper = SELECT(def._all,
                userDef.name.as(OnlineUserVO::getName),
        ...
        params.forEach((key, rv) -> {
            ...
            switch (key) {
                case ParamsConst.SEARCH_KEY -> sqlWrapper.WHERE(userDef.name.like(value).or().username.like(value));
     ...
     */
    @Override
    protected QueryField<?> doSort(OrderItem orderItem, OnlineUserDef def) {
        return switch (orderItem.getSortBy()) {
            case "loginTime" -> def.loginTime;
            case "expireTime" -> def.expireTime;
            default -> null;
        };
    }
     ...
     */
    @Override
    protected List<DefaultOrderBy> defaultSort(OnlineUserDef def) {
        // 使用以下方法构建默认排序
         return new OrderBuilder().orderDesc(def.loginTime).build();
    }
}
