package cn.icframework.system.module.chatuser.wrapperbuilder;
...
public class ChatUserWrapperBuilder extends BasicWrapperBuilder<ChatUserDef> {
     ...
     * 返回一个数据库定义对象
     */
    public ChatUserWrapperBuilder() {
        super(ChatUserDef.table());
    }
     ...
     */
    @Override
    protected SqlWrapper list(QueryParams params, ChatUserDef def) {
     ...
     * @return
     */
    public SqlWrapper allUser(String chatId, String searchKey) {
        ChatUserDef chatUserDef = ChatUserDef.table();
        UserDef userDef = UserDef.table();
        SqlWrapper sqlWrapper = SELECT(userDef)
                .FROM(chatUserDef)
                .INNER_JOIN(userDef).ON(chatUserDef.userId.eq(userDef.id))
                .WHERE(chatUserDef.chatId.eq(chatId));
        if (StringUtils.hasLength(searchKey)) {
            sqlWrapper.WHERE(userDef.name.like(searchKey));
        }
        return sqlWrapper;
    }
     ...
     */
    @Override
    protected QueryField<?> doSort(OrderItem orderItem, ChatUserDef def) {
     ...
     */
    @Override
    protected List<DefaultOrderBy> defaultSort(ChatUserDef def) {
        // 使用以下方法构建默认排序
        return new OrderBuilder().orderDesc(def.id).build();
    }
}
