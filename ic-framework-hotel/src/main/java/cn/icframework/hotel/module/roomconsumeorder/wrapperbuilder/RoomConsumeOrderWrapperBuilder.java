package cn.icframework.hotel.module.roomconsumeorder.wrapperbuilder;
...
import cn.icframework.hotel.module.consumeitem.def.ConsumeItemDef;
 ...
 */
@Component
public class RoomConsumeOrderWrapperBuilder extends BasicWrapperBuilder<RoomConsumeOrderDef> {

    /**
     * 返回一个数据库定义对象
     */
    public RoomConsumeOrderWrapperBuilder() {
        super(RoomConsumeOrderDef.table());
    }
     ...
     */
    @Override
    protected SqlWrapper list(QueryParams params, RoomConsumeOrderDef def) {
        ConsumeItemDef consumeItemDef = ConsumeItemDef.table();
        SqlWrapper sqlWrapper = SELECT(def._all,
                consumeItemDef.fileUrl.as(RoomConsumeOrderVO::getFileUrl),
                consumeItemDef.name.as(RoomConsumeOrderVO::getCustomerItemName))
                .FROM(def)
                .INNER_JOIN(consumeItemDef).ON(consumeItemDef.id.eq(def.customerItemId));
        params.forEach((key, rv) -> {
            ...
            switch (key) {
                ...
                case "roomOrderId" -> sqlWrapper.WHERE(def.roomOrderId.eq(value));
            }
        });
        return sqlWrapper;
    }
     ...
     */
    @Override
    protected QueryField<?> doSort(OrderItem orderItem, RoomConsumeOrderDef def) {
     ...
     */
    @Override
    protected List<DefaultOrderBy> defaultSort(RoomConsumeOrderDef def) {
        // 使用以下方法构建默认排序
