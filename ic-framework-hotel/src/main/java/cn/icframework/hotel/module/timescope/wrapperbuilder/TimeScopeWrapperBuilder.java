package cn.icframework.hotel.module.timescope.wrapperbuilder;
 ...
 */
@Component
public class TimeScopeWrapperBuilder extends BasicWrapperBuilder<TimeScopeDef> {
     ...
     * 返回一个数据库定义对象
     */
    public TimeScopeWrapperBuilder() {
        super(TimeScopeDef.table());
    }
     ...
     */
    @Override
    protected SqlWrapper list(QueryParams params, TimeScopeDef def) {
        DeptDef deptDef = DeptDef.table();
        SqlWrapper sqlWrapper = SELECT(def._all, deptDef.name.as(TimeScopeVO::getDeptName))
                .FROM(def)
                .LEFT_JOIN(deptDef).ON(deptDef.id.eq(def.deptId));
        params.forEach((key, rv) -> {
            ...
            switch (key) {
                ...
                case BuildColumn.deptId -> sqlWrapper.WHERE(def.deptId.eq(value));
                case ParamsConst.SEARCH_KEY -> sqlWrapper.WHERE(def.name.like(value), OR(), deptDef.name.like(value));
            }
        });
        return sqlWrapper;
    }
     ...
     */
    @Override
    protected QueryField<?> doSort(OrderItem orderItem, TimeScopeDef def) {
     ...
     */
    @Override
    protected List<DefaultOrderBy> defaultSort(TimeScopeDef def) {
        // 使用以下方法构建默认排序
