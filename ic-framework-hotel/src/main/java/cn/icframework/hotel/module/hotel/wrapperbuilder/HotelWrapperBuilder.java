package cn.icframework.hotel.module.hotel.wrapperbuilder;
 ...
 */
@Component
public class HotelWrapperBuilder extends BasicWrapperBuilder<HotelDef> {

    /**
    * 返回一个数据库定义对象
    */
    public HotelWrapperBuilder() {
        super(HotelDef.table());
    }
     ...
     */
    @Override
    protected SqlWrapper list(QueryParams params, HotelDef def) {
        UserDef userDef = UserDef.table();
        SqlWrapper sqlWrapper = SELECT(def._all, userDef.name.as(HotelVO::getManagerUserName))
                .FROM(def)
                .LEFT_JOIN(userDef).ON(userDef.id.eq(def.managerUserId));
     ...
     */
    @Override
    protected QueryField<?> doSort(OrderItem orderItem, HotelDef def) {
     ...
     */
    @Override
    protected List<DefaultOrderBy> defaultSort(HotelDef def) {
         // 使用以下方法构建默认排序
