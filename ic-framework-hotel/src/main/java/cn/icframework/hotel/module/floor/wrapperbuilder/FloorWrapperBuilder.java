package cn.icframework.hotel.module.floor.wrapperbuilder;
 ...
 */
@Component
public class FloorWrapperBuilder extends BasicWrapperBuilder<FloorDef> {

    /**
    * 返回一个数据库定义对象
    */
    public FloorWrapperBuilder() {
        super(FloorDef.table());
    }
     ...
     */
    @Override
    protected SqlWrapper list(QueryParams params, FloorDef def) {
        BuildingDef buildingDef = BuildingDef.table();
        SqlWrapper sqlWrapper = SELECT(def._all, buildingDef.name.as(FloorVO::getBuildingName))
                .FROM(def)
                .LEFT_JOIN(buildingDef).ON(buildingDef.id.eq(def.buildingId));
        params.forEach((key, rv) -> {
            ...
            switch (key) {
                 ...
                 case "buildingId" -> sqlWrapper.WHERE(def.buildingId.eq(value));
            }
        });
        return sqlWrapper;
    }
     ...
     */
    @Override
    protected QueryField<?> doSort(OrderItem orderItem, FloorDef def) {
     ...
     */
    @Override
    protected List<DefaultOrderBy> defaultSort(FloorDef def) {
         // 使用以下方法构建默认排序
