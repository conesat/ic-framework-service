package cn.icframework.hotel.module.building.wrapperbuilder;
 ...
 */
@Component
public class BuildingWrapperBuilder extends BasicWrapperBuilder<BuildingDef> {

    /**
    * 返回一个数据库定义对象
    */
    public BuildingWrapperBuilder() {
        super(BuildingDef.table());
    }
     ...
     */
    @Override
    protected SqlWrapper list(QueryParams params, BuildingDef def) {
        HotelDef hotelDef = HotelDef.table();
        SqlWrapper sqlWrapper = SELECT(def._all, hotelDef.name.as(BuildingVO::getHotelName))
                .FROM(def)
                .LEFT_JOIN(hotelDef).ON(hotelDef.id.eq(def.hotelId));
        params.forEach((key, rv) -> {
            ...
            switch (key) {
                 ...
                 case "hotelId" -> sqlWrapper.WHERE(def.hotelId.eq(value));
            }
        });
        return sqlWrapper;
    }
     ...
     */
    @Override
    protected QueryField<?> doSort(OrderItem orderItem, BuildingDef def) {
     ...
     */
    @Override
    protected List<DefaultOrderBy> defaultSort(BuildingDef def) {
         // 使用以下方法构建默认排序
