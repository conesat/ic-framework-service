package cn.icframework.hotel.module.supporting.wrapperbuilder;
 ...
 */
@Component
public class SupportingWrapperBuilder extends BasicWrapperBuilder<SupportingDef> {

    /**
    * 返回一个数据库定义对象
    */
    public SupportingWrapperBuilder() {
        super(SupportingDef.table());
    }
     ...
     */
    @Override
    protected SqlWrapper list(QueryParams params, SupportingDef def) {
     ...
     */
    @Override
    protected QueryField<?> doSort(OrderItem orderItem, SupportingDef def) {
     ...
     */
    @Override
    protected List<DefaultOrderBy> defaultSort(SupportingDef def) {
         // 使用以下方法构建默认排序
