package cn.icframework.hotel.module.consumeitem.wrapperbuilder;
 ...
 */
@Component
public class ConsumeItemWrapperBuilder extends BasicWrapperBuilder<ConsumeItemDef> {

    /**
    * 返回一个数据库定义对象
    */
    public ConsumeItemWrapperBuilder() {
        super(ConsumeItemDef.table());
    }
     ...
     */
    @Override
    protected SqlWrapper list(QueryParams params, ConsumeItemDef def) {
     ...
     */
    @Override
    protected QueryField<?> doSort(OrderItem orderItem, ConsumeItemDef def) {
     ...
     */
    @Override
    protected List<DefaultOrderBy> defaultSort(ConsumeItemDef def) {
         // 使用以下方法构建默认排序
