    ...
    <t-card class="list-card-container" :bordered="false">
      <t-row justify="space-between">
        <div class="left-operation-container">
          <t-button :disabled="!selectedRowKeys.length" @click="handleClickDelete()"
          >标记已读
          </t-button>
        ...
        <div class="search-input">
          <t-input v-model="queryForm.searchKey" placeholder="查找标题或者发布者" clearable @enter="getData(true)">
      ...
      >
        ...
        <template #op="{ row }">
          <t-space>
            ...
            <t-link v-if="!row.readTime" hover="color" theme="warning" @click="handleClickDelete(row)">标记已读</t-link>
            <t-link v-if="row.readTime" hover="color" theme="warning" @click="handleClickDelete(row)">删除</t-link>
          </t-space>
        </template>
      </t-table>
    </t-card>
...
const columns: PrimaryTableCol[] = [
  ...
  {title: '发布者', colKey: 'userName'},
  {title: '已读时间', colKey: 'readTime'},
  {title: '发布时间', colKey: 'createTime'},
  {
    align: 'left',
    fixed: 'right',
    width: 120,
