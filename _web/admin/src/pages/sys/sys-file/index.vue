<template>
  <div>
    <t-card class="list-card-container" :bordered="false">
      ...
      >
        <template #bucketUrl="{ row }">
          <t-link target="_blank" hover="color" :href="row.bucketUrl + '/' + row.ossObjectName">
            {{ row.bucketUrl + '/' + row.ossObjectName }}
          </t-link>
        </template>
        <template #type="{ row }">
          <span> {{ row.type ? row.type.text : '' }}</span>
        </template>
        <template #op="{ row }">
          <t-space>
...
const columns: PrimaryTableCol[] = [
  ...
  {title: '文件名称', colKey: 'name', ellipsis: true},
  {title: '文件大小', colKey: 'sizeTip'},
  {title: 'url', colKey: 'bucketUrl', ellipsis: true},
  {title: '文件系统类型', colKey: 'type'},
  {title: '引用次数', colKey: 'refCount', width: 100},
...
const handleClickEdit = (id?: any) => {
  router.push(`/file/sys-file-edit?id=${id ? id : ''}`);
};
// 单个删除按钮事件
