  ...
  <div>
    <t-card class="list-card-container" :bordered="false">
      ...
      >
        <template #op="{ row }">
          <t-space>
            <t-link hover="color" theme="primary" @click="handleClickFloor(row)">查看楼层</t-link>
...
const emit = defineEmits(['showFloor']);
// 定义变量 start -------------------
...
const handleClickEdit = (id?: any) => {
  router.push(`/hotel/building-edit?autoClose=true&hotelId=${props.hotelId}&id=${id ? id : ''}`);
};
const handleClickFloor = (item: any) => {
  emit('showFloor', item);
};
// 单个删除按钮事件
