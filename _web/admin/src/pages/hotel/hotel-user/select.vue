<template>
  <div>
    <t-card class="list-card-container" :bordered="false">
      ...
      >
        <template #vip="{ row }">
          <t-tag :theme="row.vip?'success':'default'">{{ row.vip ? '是' : '否' }}</t-tag>
        </template>
        <template #status="{ row }">
...
const columns: PrimaryTableCol[] = [
  ...
  {title: '头像', colKey: 'avatarFileUrl'},
  {title: '名称', colKey: 'name'},
  {title: '手机号', colKey: 'phone'},
  {title: '是否vip', colKey: 'vip'},
  {title: '是否有效', colKey: 'status'},
  {title: '累计消费金额', colKey: 'totalConsume'},
];
// 定义变量 end -------------------
...
const handleClickEdit = () => {
  window.open(`#/hotel/hotel-user-edit?autoClose=true`);
};
// 定义方法 end -------------------
