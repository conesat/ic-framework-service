<template>
  <div>
    <t-card class="list-card-container" :bordered="false">
      ...
      >
        <template #iconFileUrl="{ row }">
          <t-image
            style="background-color: rgba(255,255,255,0.2);width: 20px;height: 20px;border-radius: var(--td-radius-default)"
            v-if="row.iconFileUrl" :src="row.iconFileUrl"/>
        </template>
        <template #supportingType="{ row }">
          <t-tag v-if="row.supportingType" variant="light">{{ row.supportingType.text }}
          </t-tag>
        </template>
        <template #op="{ row }">
          <t-space>
...
const columns: PrimaryTableCol[] = [
  ...
  {title: '图标', colKey: 'iconFileUrl'},
  {title: '配套设施类型', colKey: 'supportingType'},
  {
    align: 'left',
    fixed: 'right',
    width: 120,
...
const handleClickEdit = (id?: any) => {
  router.push(`/hotel/supporting-edit?id=${id ? id : ''}`);
};
// 单个删除按钮事件
