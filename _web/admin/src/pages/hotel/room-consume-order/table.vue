<template>
  <div>
    <t-card class="list-card-container" :bordered="false">
      <t-row justify="space-between">
        <div class="left-operation-container">
          <t-button @click="(e)=>{roomConsumeOrderId='';showRoomConsumeOrder = true;}">新建</t-button>
        ...
        <div class="search-input">
          <t-input v-model="queryForm.searchKey" placeholder="请输入你需要搜索的内容" clearable @enter="getData()">
            <template #suffix-icon>
              <search-icon size="16px" @click.stop="getData()"/>
            </template>
          </t-input>
        </div>
      </t-row>
      ...
      >
        <template #status="{row}">
          <t-tag :theme="row.status.code ? 'success':'default'">{{ row.status.text }}</t-tag>
        </template>
        <template #op="{ row }">
          <t-space>
    ...
    <t-dialog v-model:visible="showRoomConsumeOrder" header="创建订单" :footer="false" width="800">
      <room-consume-order-edit v-if="showRoomConsumeOrder"
                               :room-order-id="roomOrderId"
                               :id="roomConsumeOrderId"
                               @success="(e)=>{showRoomConsumeOrder=false;getData(true);}"
      />
    </t-dialog>
  </div>
</template>
...
const roomConsumeOrderId = ref('')
// 表头
const columns: PrimaryTableCol[] = [
  {colKey: 'row-select', type: 'multiple', width: 64, fixed: 'left'},
  {title: '项目', colKey: 'customerItemName'},
  {title: '金额', colKey: 'price'},
  {title: '数量', colKey: 'num'},
  {title: '状态', colKey: 'status'},
  {title: '支付时间', colKey: 'payTime'},
  {title: '创建时间', colKey: 'createTime'},
  {title: '更新时间', colKey: 'updateTime'},
  {
    align: 'left',
    fixed: 'right',
    width: 120,
...
const getData = async (reload?: boolean) => {
  ...
  queryForm.value.roomOrderId = props.roomOrderId;
  ApiRoomConsumeOrder.page({
