<template>
  <div>
    <t-card class="list-card-container" :bordered="false">
      <t-row justify="space-between">
        <div class="left-operation-container">
          ...
          <t-button variant="base" theme="default" :disabled="!selectedRowKeys.length" @click="showSelectUser = true">
            派单
          </t-button>
          <t-button variant="base" theme="warning" :disabled="!selectedRowKeys.length" @click="handleClickDelete()">
            删除
          </t-button>
      ...
      >
        <template #state="{ row }">
          <t-tag
            :theme="row.state.code === 0 ? 'default':(row.state.code === 2 ? 'primary':(row.state.code === 3 ? 'success':'warning'))">
            {{ row.state.text }}
          </t-tag>
        </template>
        <template #target="{ row }">
          <t-tag>{{ row.target.text }}</t-tag>
        </template>
        <template #op="{ row }">
          <t-space>
    ...
    <t-dialog v-model:visible="showSelectUser" header="选择指派人员" :footer="false" width="800"
...
const columns: PrimaryTableCol[] = [
  ...
  {title: '服务状态', colKey: 'state'},
  {title: '服务类型', colKey: 'target'},
  {title: '接单人员', colKey: 'serverUserName'},
  {title: '生成时间', colKey: 'createTime'},
  {title: '接单时间', colKey: 'acceptTime'},
  {title: '期望完成时间', colKey: 'doTime'},
  {title: '完成时间', colKey: 'finishTime'},
  {
    align: 'left',
    fixed: 'right',
    width: 120,
...
const confirmDeleteBody = () => {
  if (selectedRowData.value.length > 0) {
    return `确认删除所选 ${selectedRowData.value.length} 个数据吗？，删除后无法恢复`;
  }
  return '';
};
// 确认删除事件
...
const handleClickEdit = (id?: any) => {
  router.push(`/hotel/server-order-edit?id=${id ? id : ''}`);
};
// 单个删除按钮事件
