...
export const useTabsRouterStore = defineStore('tabsRouter', {
  ...
  actions: {
    ...
    // 处理关闭当前
    closeTab(route: RouteLocationNormalizedLoaded, router: Router) {
      const options = {
        index: 0,
        value: route.path,
      };
      for (let i = this.tabRouters.length - 1; i > 0; i--) {
        const tab = this.tabRouters[i]
        if (tab.path === route.path) {
          options.index = i;
          break
        }
      }
      const nextRouter = this.tabRouters[options.index - 1] || this.tabRouters[options.index + 1];
      this.subtractCurrentTabRouter({ path: options.value as string, routeIdx: options.index });
