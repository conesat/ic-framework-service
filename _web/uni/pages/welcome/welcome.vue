  ...
  <view class="login-page">
    ...
    <view class="login-sheet">
      ...
      <view class="login-heading">
        ...
        <text class="login-desc">未注册的手机号将自动创建账户</text>
      </view>
      ...
      <view class="form-card">
        ...
        <view class="field-row">
          ...
          <input v-model="code" class="field-input" type="number" maxlength="6" placeholder="请输入验证码" placeholder-class="placeholder" />
          <view class="code-button" :class="{ disabled: codeCountdown > 0 }" @tap="getCode">
...
export default {
  ...
  computed: {
    canLogin() {
      return /^1\d{10}$/.test(this.phone) && this.code.length >= 4 && this.agreed
    }
  },
  onUnload() {
  ...
  methods: {
    ...
    login() {
      if (!this.canLogin) {
        uni.showToast({ title: '请填写手机号、验证码并阅读协议', icon: 'none' })
        return
      }
      this.enterHome()
    },
